...
try:
    if 'Handset Type' in data.columns:
        ...
        data['Handset Type'].value_counts().head(10).plot(kind='bar')
        plt.tight_layout()
        plt.savefig("handset_bar.png",dpi=300)
        plt.close()

    if 'Handset Manufacturer' in data.columns:
        plt.figure(figsize=(12,6))
        data['Handset Manufacturer'].value_counts().head(10).plot(kind='bar')
        plt.tight_layout()
        plt.savefig("manufacturer_bar.png",dpi=300)
        plt.close()
    ...
    if 'Total DL (Bytes)' in data.columns and 'Total UL (Bytes)' in data.columns:
        ...
        plt.scatter(data['Total DL (Bytes)'],data['Total UL (Bytes)'],s=4)
        plt.tight_layout()
        plt.savefig("download_upload_scatter.png",dpi=300)
        plt.close()
